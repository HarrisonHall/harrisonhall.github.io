...
class StatisticsWorkers
    ...
    newWorker: () ->
        return new Threads.Worker @worker
    ...
    worker: () ->
        ...
        getRatings = (tables) ->
            ...
            filteredPlayers = tables.players
                .filter((i) -> i.lastGame.getTime() > oneMonthAgo)
            for player in filteredPlayers.sort((a, b) -> b.overall - a.overall)
                if player.num_games > 15 and not player.stats_hidden
                    color = "hsl(0,0%,#{if player.num_games < 250 then (100-(100*(player.num_games/312.5)+20))+"%" else "100%"})"
                    html += "<tr style='color:#{color};'><td>#{player.name}</td><td>#{player.res}</td><td>#{player.spy}</td><td>#{player.overall}</td></tr>"
        ...
        this.onmessage = (e) ->
            ...
            postMessage {stats:stats, tables: tables}
